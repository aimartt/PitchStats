...
const MatchList: React.FC<MatchListProps> = ({ matches, onNavigate, currentUserRole, seasons, onEditMatch, onViewMatch, currentTeamName }) => {
  ...
  const getWatermarkIcon = (matchType?: string) => {
    switch(matchType) {
       case '联赛': return <Trophy className="absolute right-[-10px] bottom-[-20px] w-40 h-40 text-emerald-100/50 -rotate-12 pointer-events-none" />;
       case '杯赛': return <Crown className="absolute right-[-10px] bottom-[-20px] w-40 h-40 text-amber-100/50 -rotate-12 pointer-events-none" />;
       case '友谊赛': return <Handshake className="absolute right-[-10px] bottom-[-20px] w-40 h-40 text-sky-100/50 -rotate-12 pointer-events-none" />;
       case '队内赛': return <Shirt className="absolute right-[-10px] bottom-[-20px] w-40 h-40 text-slate-200/50 -rotate-12 pointer-events-none" />;
       default: return null;
    }
  };
        ...
        {filteredMatches.length > 0 ? (
          filteredMatches.map((match, idx) => (
            ...
            >
               ...
               {/* Card Content */}
               <div className="pl-5 pr-4 py-4 md:pl-8 md:pr-6 md:py-5 w-full relative z-10">
                  
                  {/* Top Meta Row */}
                  <div className="flex flex-wrap items-center justify-between text-xs text-slate-500 mb-4 pb-3 border-b border-slate-900/5 gap-y-2">
                  ...
                  <div className="flex items-center justify-between mb-4">
                      ...
                      <div className="flex-1 text-right flex items-center justify-end gap-3">
                         <span className="font-bold text-slate-900 text-base md:text-lg truncate hidden md:block" style={{ color: 'var(--primary-text)' }}>
                           {currentTeamName || '我方球队'}
                         </span>
                         <span className="font-bold text-slate-900 text-sm md:hidden truncate" style={{ color: 'var(--primary-text)' }}>
                           {currentTeamName || '我方'}
                         </span>
                         <div className="w-10 h-10 md:w-12 md:h-12 rounded-full bg-white border border-slate-200 flex items-center justify-center font-bold text-sm md:text-base text-slate-500 shrink-0 shadow-sm">
                            H
                         </div>
                      </div>

                      {/* Score Badge (High Contrast) */}
                      <div className={`mx-3 md:mx-6 px-4 py-2 rounded-xl border flex flex-col items-center min-w-[100px] shadow-sm transform group-hover:scale-105 transition-transform ${getResultColorClass(match.result)}`}>
                         <div className="text-2xl md:text-3xl font-black font-mono leading-none tracking-tight">
                            {match.matchType === '队内赛' && match.ourScore === 0 && match.opponentScore === 0 ? (
                               <span className="text-sm font-sans font-bold uppercase tracking-wider">Training</span>
                            ) : (
                               `${match.ourScore} : ${match.opponentScore}`
                            )}
                         </div>
                         {match.matchType !== '队内赛' && (
                           <span className="text-[10px] font-bold uppercase mt-1 opacity-90 tracking-widest border-t border-white/20 pt-0.5 w-full text-center">
                      ...
                      <div className="flex-1 text-left flex items-center justify-start gap-3">
                         <div className="w-10 h-10 md:w-12 md:h-12 rounded-full bg-white border border-slate-200 flex items-center justify-center font-bold text-sm md:text-base text-slate-500 shrink-0 overflow-hidden shadow-sm">
                  ...
                  {( (match.goalsDetails && match.goalsDetails.length > 0) || 
                     ...
                     <div className="mt-4 pt-3 border-t border-dashed border-slate-200/60 space-y-2">
                        {/* Goals */}
                        {match.goalsDetails && match.goalsDetails.length > 0 && (
                  ...
                  <div className="flex items-center justify-between pt-1 mt-2">
                      ...
                      <div className="flex items-center gap-2">
                        {/* View Detail Prompt */}
                        <span className="text-xs text-slate-400 group-hover:text-slate-600 flex items-center transition-colors">
                        ...
                        {currentUserRole === 'admin' && onEditMatch && (
                           <button 
                              ...
                              className="ml-3 text-xs font-bold text-slate-500 hover:text-blue-600 flex items-center gap-1 px-3 py-1.5 bg-white hover:bg-blue-50 border border-slate-200 hover:border-blue-200 rounded-lg transition-all shadow-sm z-20"
                           >
